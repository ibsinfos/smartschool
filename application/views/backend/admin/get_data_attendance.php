       ...
       <?php  foreach($attendance as $row){ ?>
		...
		<input type="checkbox" name="delete_id[]" value="<?php echo $row->attendance_id; ?>" class="checkbox1" /></td>
			<td><?php echo date("F d, Y",strtotime($row->date)); ?></td>
				  ...
				  <td><div class="btn-group"><button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">Action <span class="caret"></span></button><ul class="dropdown-menu dropdown-default pull-right" role="menu"><li><a href="<?php echo base_url().'index.php?admin/student_attendance/delete/'.$row->attendance_id; ?>"><i class="entypo-trash"></i>Remove</a></li></ul></div></td></tr>
		<?php } ?>
		</tbody>
        </table>
        <table>
                <tr>
                <td><input type="submit" name="delete_all" id="deletesall"  class="btn btn-info" value="Remove All"  /></td>
                </tr>
                </table>
		<?php }
		if(isset($month))
