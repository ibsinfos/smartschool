			...
			<tbody>
				...
				<tr>
					<td><?php
						...
						{
							$parentid=$this->db->get_where('parent',array('parent_id'=>$user_to_show_id))->result_array();
							
							$studentdata=$this->db->get_where('student',array('parent_email'=>$parentid[0]['parent_email']))->result_array();
							foreach ($studentdata as $s)
							{
								$stuname.=$s['name'].',';
							}
						?>
						<label  title="<?=$stuname?>"><?=$parentid[0]['name']?></label>
						<?php 
						}
						else
						{
...
<script type="text/javascript">
	...
	$("#dropdownMenu2").change(function () {
		...
		$.ajax({   
			...
			url: "index.php?getinfo/getinfo", //The url where the server req would we made.
