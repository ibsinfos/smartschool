<div class="sidebar-menu">
    <header class="logo-env" >
        ...
        <div class="logo" style="">
            <a href="<?php echo base_url(); ?>">
               <?php $image_admin=$this->db->get_where('parent', array('parent_id'=>$this->session->userdata('parent_id')))->row();
			   if($image_admin->parent_image == ""){
			   ?>
               <img width="60" height="60" class="img-circle" src="<?php echo base_url(); ?>/uploads/user.jpg"><?php }else {?>
               <img width="60" height="60" class="img-circle" src="<?php echo base_url(); ?>/uploads/parent_image/<?php echo $image_admin->parent_image; ?>"><?php }?>
            </a>
        </div>
	...
	<?php
		 $display="";
		// echo "==============>".$account_type;
		$run = "FIND_IN_SET('".$this->session->userdata('parent_id')."', user_role)";
		$this->db->where($run);
		...
		//print_r($user_role_query);
		  //die;
    if(in_array($page_name,$final_module_assgin) || $page_name=='dashboard' ||$page_name=='manage_profile' || $page_name=='notification' || $page_name=='reports'||$page_name=='message'){	
			?>
