            ...
            <div class="tab-pane box active" id="list" style="width:100%;">
                <table class="table datatable table" border="0" cellpadding="0" cellspacing="0" style="width:100%; font-family:Arial, Helvetica, sans-serif; font-size:14px; letter-spacing:0.5;" id="table_export">
                	<thead>
                       ...
                       <tr bgcolor="#000000"><td colspan="6" style="font-size:1.2em; text-align:center; font-weight:600;  color:#FFF;">Subject Wise Toppers</td></tr>
                         <tr bgcolor="#000000">
                		   <td colspan="6" style="font-size:1.0em; text-align:center; font-weight:600; padding:6px 0px; color:#FFF;">
                            <?php 							
				echo ucwords("Standard : <span style='text-align:right;color:#2092D0;font-size:1.0em;font-weight:600;'>".$standard."</span>"); ?>
                            </td>
                        </tr>      
                		
                        <tr bgcolor="#2092D0">
                    		<th style="color:#FFF;  text-align:center;" width="20%">Student Name</th>
                            <th style="color:#FFF; padding:6px 0px; border-right:1px solid #EEE;  text-align:center;" width="20%">Class Name</th>
                                	<th style="color:#FFF; padding:6px 0px; border-right:1px solid #EEE;  text-align:center;" width="20%">Subject Name</th>
                    		<th style="color:#FFF;  text-align:center; " width="40%">Marks</th>
                    		<th style="color:#FFF;  text-align:center; " width="40%">out of marks</th>
                     	</tr>
					</thead>
                    <tbody>
                    	<?php 
					...
					$this->db->limit(3, 0);
					$mark = $this->db->get_where('mark',array('subject_id'=>$subject,'class_id'=>intval($standard)))->result_array();*/
				/*$this->db->order_by("mark.mark_obtained","desc");
					$this->db->limit(3, 0);
					
					$this->db->join('exam','exam.subject_id=mark.subject_name');
					$mark = $this->db->get_where('mark',array('mark.subject_id'=>$subject,'mark.class_id'=>intval($standard)))->result_array();
					*/
					/*$this->db->order_by("mark.mark_obtained","desc");
					$this->db->limit(3, 0);
					...
					*/
					$mark = $this->db->query("SELECT DISTINCT(s1.student_id), s1.subject_id, s2.mks, s1.student_name, e.out_of_marks, s1.class_id FROM mark s1, ( SELECT s.subject_id, MAX( s.mark_obtained ) AS mks,s.subject_name FROM mark s GROUP BY s.subject_id )s2 left join exam e ON e.subject_id = s2.subject_name WHERE s1.subject_id = s2.subject_id AND s1.mark_obtained = s2.mks AND s1.class_id =".intval($standard)." ORDER BY s1.subject_id ASC")->result_array();
						...
						$subjects = $this->db->get_where("subject",array('subject_id'=>$row_mark['subject_id']))->row();
							
					
						?>
                        <tr>
                            <td align="center" style="font-size:11px;  border-bottom:1px solid #EEE; border-left:1px solid #EEE; border-right:1px solid #EEE;" width="10%"><?php echo  $row_mark['subject_id'];?></td>
							...
							<td align="center" style="font-size:11px;  border-bottom:1px solid #EEE; border-left:0px solid #EEE; border-right:0px solid #EEE;" width="25%" ><?php echo $row_mark['mks'];?></td>
